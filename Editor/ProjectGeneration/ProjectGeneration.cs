    ...
    public class ProjectGeneration : IGenerator
    {
        ...
        )
        {
            using (solutionSyncMarker.Auto())
            {
                ...
                var reimported =
                    reimportedFiles as ICollection<string> ?? reimportedFiles.ToArray();
                ...
                var affectedAndReimported = new HashSet<string>(
                    affectedNames.Concat(reimportedNames)
                );
        ...
        private bool HasFilesBeenModified(
            IEnumerable<string> affectedFiles,
            IEnumerable<string> reimportedFiles
        )
        {
        ...
        private void RefreshCurrentInstallation()
        {
            ...
            editor?.TryGetAntigravityInstallationForPath(
        ...
        internal virtual void IncludeAsset(StringBuilder builder, IncludeAssetTag tag, string asset)
        {
            ...
            if (Path.IsPathRooted(filename) && packageInfo != null)
            {
                ...
                var linkPath = SkipPathPrefix(
                    asset.NormalizePathSeparators(),
        ...
        {
            foreach (
                var method in TypeCacheHelper.GetPostProcessorCallbacks(
                    nameof(OnGeneratedCSProjectFiles)
                )
            )
            {
        ...
        private static bool OnPreGeneratingCSProjectFiles()
        {
            ...
            foreach (
                var method in TypeCacheHelper.GetPostProcessorCallbacks(
                    nameof(OnPreGeneratingCSProjectFiles)
                )
            )
            {
        ...
        )
        {
            ...
            var responseRefs = responseFileData.SelectMany(x =>
                x.FullPathReferences.Select(r => r)
            );
        ...
        internal string GetLangVersion(Assembly assembly, ResponseFileData[] responseFileData)
        {
            ...
            if (m_CurrentInstallation != null)
            {
                ...
                var unityLanguageSupport = UnityInstallation.LatestLanguageVersionSupported(
                    assembly
                );

                // Use the minimal supported version between VS and Unity, so that compilation will work in both
                targetLanguageVersion = (
                    vsLanguageSupport <= unityLanguageSupport
                        ? vsLanguageSupport
                        : unityLanguageSupport
        ...
        )
        {
            ...
            additionalFilePaths.AddRange(
                afpProperty?.GetValue(compilerOptions) as string[] ?? Array.Empty<string>()
            );
            analyzerConfigPath =
                acpProperty?.GetValue(compilerOptions) as string ?? analyzerConfigPath;
        ...
        )
        {
            ...
            var projectProperties = new ProjectProperties
            {
                ...
                // RSP alterable
                Defines = assembly
                    .defines.Concat(responseFileData.SelectMany(x => x.Defines))
                    .Distinct()
                    .ToArray(),
                Unsafe =
                    assembly.compilerOptions.AllowUnsafeCode | responseFileData.Any(x => x.Unsafe),
                // VSTU Flavoring
        ...
        internal virtual void GetProjectHeader(
            ProjectProperties properties,
            out StringBuilder headerBuilder
        )
        {
        ...
        internal void GetProjectHeaderProperties(
            ProjectProperties properties,
        ...
        internal static void GetProjectHeaderAnalyzers(
            ProjectProperties properties,
            StringBuilder headerBuilder
        )
        {
        ...
        private void SyncSolution(IEnumerable<Assembly> assemblies)
        {
            if (InvalidCharactersRegexPattern.IsMatch(ProjectDirectory))
                Debug.LogWarning(
                    "Project path contains special characters, which can be an issue when opening Antigravity"
                );
        ...
        internal virtual string SolutionText(
        ...
        )
        {
            ...
            string projectConfigurationsText = string.Join(
                ...
                configurableProjects
                    .Select(p => GetProjectActiveConfigurations(p.ProjectGuid))
                    .ToArray()
            );
        ...
        )
        {
            ...
            if (previousSolution != null)
            {
                ...
                var externalProjects = previousSolution
                    .Projects.Where(p =>
                        p.IsSolutionFolderProjectFactory()
        ...
        private static IEnumerable<Assembly> RelevantAssembliesForMode(
            IEnumerable<Assembly> assemblies
        )
        {
        ...
        internal string EscapedRelativePathFor(
            string file,
        ...
        {
            if (
                path.StartsWith($"{prefix}{Path.DirectorySeparatorChar}")
