...
namespace Antigravity.Unity.Editor
{
    ...
    {
        private static readonly SdkStyleProjectGeneration _sdkStyleProjectGeneration =
            new SdkStyleProjectGeneration();
        ...
        public static IGenerator GetInstance(GeneratorStyle style)
        {
            ...
            if (
                forceStyleString != null
                && Enum.TryParse<GeneratorStyle>(forceStyleString, out var forceStyle)
            )
                style = forceStyle;
        ...
        private static string OnSelectingCSProjectStyle()
        {
            foreach (
                var method in TypeCacheHelper.GetPostProcessorCallbacks(
                    nameof(OnSelectingCSProjectStyle)
                )
            )
            {
