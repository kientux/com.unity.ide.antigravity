...
namespace Antigravity.Unity.Editor
{
    ...
    {
        private static readonly IGenerator _generator = GeneratorFactory.GetInstance(
            GeneratorStyle.SDK
        );
        ...
        private static bool IsCandidateForDiscovery(string path)
        {
            ...
            return Directory.Exists(path)
                && Regex.IsMatch(path, ".*Antigravity.*.app$", RegexOptions.IgnoreCase);
#elif UNITY_EDITOR_WIN
            return File.Exists(path)
                && Regex.IsMatch(path, ".*Antigravity.*.exe$", RegexOptions.IgnoreCase);
#else
        ...
        )
        {
            ...
            try
            {
                ...
                var manifestFullPath = IOPath.Combine(
                    manifestBase,
                    "resources",
                    "app",
                    "package.json"
                ...
                {
                    var manifest = JsonUtility.FromJson<AntigravityManifest>(
        ...
        public static IEnumerable<IAntigravityInstallation> GetAntigravityInstallations()
        {
            ...
            var programFiles = IOPath.Combine(
        ...
        public void CreateExtraFiles(string projectDirectory)
        {
            try
            {
                var vscodeDirectory = IOPath.Combine(
                    projectDirectory.NormalizePathSeparators(),
                    ".vscode"
                );
                Directory.CreateDirectory(vscodeDirectory);

                var enablePatch = !File.Exists(
                    IOPath.Combine(vscodeDirectory, ".vstupatchdisable")
                );
...
";
        ...
        private static void CreateRecommendedExtensionsFile(
        ...
        private static void PatchRecommendedExtensionsFile(string extensionFile)
        {
            try
            {
                ...
                if (
                    recommendations.Linq.Any(entry =>
                        entry.Value.Value == MicrosoftUnityExtensionId
                    )
                )
                    return;
        ...
        private static ProcessStartInfo ProcessStartInfoFor(string application, string arguments)
        {
            ...
            application = "open";
            return ProcessRunner.ProcessStartInfoFor(
                application,
                arguments,
                redirect: false,
                shell: true
