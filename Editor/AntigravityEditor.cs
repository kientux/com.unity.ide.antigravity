    ...
    public class AntigravityEditor : IExternalCodeEditor
    {
        CodeEditor.Installation[] IExternalCodeEditor.Installations =>
            _discoverInstallations
                .Result.Values.Select(v => v.ToCodeEditorInstallation())
                .ToArray();

        private static readonly AsyncOperation<
            Dictionary<string, IAntigravityInstallation>
        > _discoverInstallations;
        ...
        static AntigravityEditor()
        {
            ...
            _discoverInstallations = AsyncOperation<
                Dictionary<string, IAntigravityInstallation>
            >.Run(DiscoverInstallations);
        }
        ...
        internal static bool IsEnabled =>
            CodeEditor.CurrentEditor is AntigravityEditor
        ...
        {
            if (
                !TryGetAntigravityInstallationForPath(
        ...
        internal virtual bool TryGetAntigravityInstallationForPath(
            string editorPath,
        ...
        public virtual bool TryGetInstallationForPath(
            string editorPath,
            out CodeEditor.Installation installation
        )
        {
            var result = TryGetAntigravityInstallationForPath(
                editorPath,
        ...
        private static void SettingsButton(
            ...
            string guiMessage,
            string toolTip,
            IAntigravityInstallation installation
        )
        {
            var generator = installation.ProjectGenerator;
            var prevValue = generator.AssemblyNameProvider.ProjectGenerationFlag.HasFlag(
                preference
            );
        ...
        )
        {
            ...
            foreach (var file in importedFiles.Where(a => Path.GetExtension(a) == ".pdb"))
            {
                ...
                if (
                    pdbFile.IndexOf(
                        $"{Path.DirectorySeparatorChar}com.unity.",
        ...
        public bool OpenProject(string path, int line, int column)
        {
            ...
            {
                Debug.LogWarning(
                    $"Antigravity executable {editorPath} is not found. Please change your settings in Edit > Preferences > External Tools."
                );
                return false;
            }
